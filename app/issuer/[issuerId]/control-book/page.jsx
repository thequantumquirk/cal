...
export default function ControlBookPage({ params: paramsPromise }) {
  ...
  const { data: transferTransactions = [], isLoading: transfersLoading, mutate: mutateTransferTransactions } = useSWR(
  ...
  useEffect(() => {
    if (!securitiesLoading && !transfersLoading && issuerId) {
  ...
  }, [securities, transferTransactions, securitiesLoading, transfersLoading, issuerId, activeTab]);
  ...
  const applyFilters = useCallback(() => {
    ...
    if (dateFrom) {
      filtered = filtered.filter((entry) => {
        const entryDate = entry.transaction_date || entry.created_at.split("T")[0];
                ...
                <CardContent>
                  {loading ? (
                    <div className="flex items-center justify-center py-16">
                      <div className="text-center">
                        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-500 mx-auto mb-4"></div>
                        <p className="text-gray-600">Loading control book data...</p>
                      </div>
                    </div>
