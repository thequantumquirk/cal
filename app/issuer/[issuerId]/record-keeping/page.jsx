...
export default function RecordKeepingPage({ params: paramsPromise }) {
  ...
  // ⚡ OPTIMIZED SWR CONFIG - Fresh data without compromising performance
  ...
  const swrConfig = {
    revalidateOnFocus: true,        // ✅ Refresh when returning to tab (multi-tab support)
    revalidateOnReconnect: true,    // ✅ Refresh when connection restored
    dedupingInterval: 1000,         // ✅ 1 second (allow rapid updates, prevent spam)
    revalidateIfStale: true,        // ✅ Auto-revalidate when cache marked stale
    refreshInterval: 0,             // No polling (Supabase realtime handles updates)
    focusThrottleInterval: 3000,    // Throttle focus revalidation to every 3 seconds
  };

  // ⚡ CRITICAL: Use a stable key that includes the issuerId from params if state isn't set yet
  // This prevents SWR from pausing while waiting for the useEffect to set state
  const effectiveIssuerId = issuerId || (typeof window !== 'undefined' ? window.location.pathname.split('/')[2] : null);

  const { data: securities = [], isLoading: securitiesLoading, mutate: mutateSecurities } = useSWR(
    effectiveIssuerId ? `/api/securities?issuerId=${effectiveIssuerId}` : null,
    swrFetcher,
    swrConfig
  );

  const { data: shareholders = [], isLoading: shareholdersLoading, mutate: mutateShareholders } = useSWR(
    effectiveIssuerId ? `/api/shareholders?issuerId=${effectiveIssuerId}` : null,
    swrFetcher,
    swrConfig
  );
  ...
  const { data: enrichedTransactions = [], isLoading: transactionsLoading, mutate: mutateTransactions } = useSWR(
    effectiveIssuerId ? `/api/record-keeping-transactions?issuerId=${effectiveIssuerId}` : null,
    swrFetcher,
    swrConfig
  );

  const { data: restrictionTemplates = [], isLoading: templatesLoading, mutate: mutateRestrictionTemplates } = useSWR(
    effectiveIssuerId ? `/api/restriction-templates?issuerId=${effectiveIssuerId}` : null,
    swrFetcher,
    swrConfig
  );
  ...
  useEffect(() => {
    ...
    if (searchParams.get('refresh') === 'true' && effectiveIssuerId) {
      ...
      const revalidateAll = async (source) => {
        console.log(`⚡ [${source}] Triggering SWR revalidation...`);
        await Promise.all([
          mutateTransactions(undefined, { revalidate: true }),
          mutateShareholders(undefined, { revalidate: true }),
          mutateSecurities(undefined, { revalidate: true }),
          mutateRestrictionTemplates(undefined, { revalidate: true })
        ]);
        console.log(`✅ [${source}] Revalidation completed`);
        setLastUpdated(new Date());
      };

      // 1. Immediate revalidation
      revalidateAll('IMMEDIATE').then(() => setUrlRefreshing(false));

      // 2. Delayed revalidation (2 seconds) to ensure DB consistency
      console.log('⏳ Scheduling delayed revalidation in 2s...');
      setTimeout(() => {
        revalidateAll('DELAYED-2S');
      }, 2000);
  ...
  }, [searchParams, effectiveIssuerId, mutateSecurities, mutateShareholders, mutateTransactions, mutateRestrictionTemplates]);

  // ⚡ REALTIME: Optimistic UI updates with background verification
  useEffect(() => {
    if (!issuerId) return;
        ...
        async (payload) => {
          ...
          try {
            if (payload.eventType === 'INSERT' && payload.new) {
              ...
              // 1. Force immediate cache update with new data
              await mutateTransactions(
                (currentData) => {
                  const safeData = currentData || [];
                  // Check if already exists to prevent duplicates
                  if (safeData.some(t => t.id === payload.new.id)) {
                    return safeData;
                  }

                  const updatedData = [payload.new, ...safeData];
                  // Sort by transaction_date desc to match default view
                  return updatedData.sort((a, b) => new Date(b.transaction_date) - new Date(a.transaction_date));
                },
                { revalidate: false } // Don't fetch from server yet
              );

              // 2. Trigger a hard refresh of ALL data to ensure consistency (stats, shareholders, etc.)
              // We do this after a short delay to allow DB propagation
              setTimeout(() => {
                console.log('🔄 Triggering full consistency refresh...');
                Promise.all([
                  mutateTransactions(),
                  mutateShareholders(), // Update shareholder balances
                  mutateSecurities()    // Update security totals
                ]);
              }, 1000);
            ...
            setTimeout(() => {
              ...
              setRealtimeUpdate(null); // Clear badge
            }, 1000);
          }
        }
      )
  ...
  }, [issuerId, mutateTransactions, mutateShareholders, mutateSecurities]);
  ...
  // Manual refresh handler - force SWR to refetch all data
  const handleRefresh = async () => {
    console.log('🔄 Manual refresh triggered');
    setRefreshing(true);
    try {
      // Force refetch all data from server with revalidation
      await Promise.all([
        mutateSecurities(undefined, { revalidate: true }),
        mutateShareholders(undefined, { revalidate: true }),
        mutateTransactions(undefined, { revalidate: true }),
