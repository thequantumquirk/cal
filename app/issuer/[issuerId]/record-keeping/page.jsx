...
export default function RecordKeepingPage({ params: paramsPromise }) {
  ...
  const [data, setData] = useState({
    ...
    shareholders: [],
    transactions: [],
    // restrictions: [], // ⚡ REMOVED: Never used in this page (only restrictionTemplates is used)
  ...
  // ⚡ FIXED: Prevent duplicate fetches with ref tracking
  ...
  // Reset fetch flag when issuerId changes
  useEffect(() => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // ⚡ OPTIMIZED: Removed unused shareholder-restrictions API call (saves ~1000ms)
      // Only restrictionTemplates is used in this page, not restrictions
      const [
        securitiesRes,
        shareholdersRes,
        transferJournalRes,
      ...
      console.log("🔍 Raw transactions sample:", enrichedTransactions[0]);
      console.log("🔍 Restriction templates:", restrictionTemplates);

      // ⚡ OPTIMIZED: Enrich transactions on client using already-fetched data
      // This eliminates duplicate database queries in the API
      const securitiesMap = {};
      ...
      const transactions = enrichedTransactions.map(transaction => {
        ...
        return {
          ...
          // Restriction enrichment
          restrictions: restrictionTemplate ? [restrictionTemplate] : [],
          restricted_shares: restrictionTemplate ? transaction.share_quantity : 0,
      ...
      console.log(`✅ Record-keeping page data loaded in ${duration}ms`);
    } catch (error) {
