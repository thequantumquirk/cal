...
export default function RecordKeepingPage({ params: paramsPromise }) {
  ...
  const enrichedData = useMemo(() => {
    ...
    // Calculate stats - use Math.abs because quantities may be negative in DB
    // 🔍 DEBUG: Log Class A (G17564108) transactions for debugging
    const classATransactions = transactions.filter(t => t.cusip === 'G17564108');
    console.log('🔍 === CLASS A (G17564108) DEBUG ===');
    console.log(`Total Class A transactions: ${classATransactions.length}`);

    const classACredits = classATransactions.filter(t => t.credit_debit === 'Credit');
    const classADebits = classATransactions.filter(t => t.credit_debit === 'Debit');

    console.log(`Class A Credits: ${classACredits.length} transactions`);
    console.log(`Class A Debits: ${classADebits.length} transactions`);

    const classACreditTotal = classACredits.reduce((sum, t) => sum + Math.abs(Number(t.share_quantity || 0)), 0);
    const classADebitTotal = classADebits.reduce((sum, t) => sum + Math.abs(Number(t.share_quantity || 0)), 0);

    console.log(`Class A Credit Total: ${classACreditTotal.toLocaleString()}`);
    console.log(`Class A Debit Total: ${classADebitTotal.toLocaleString()}`);
    console.log(`Class A Outstanding: ${(classACreditTotal - classADebitTotal).toLocaleString()}`);
    console.log(`Expected: 22,402,366`);
    console.log(`Difference: ${(classACreditTotal - classADebitTotal - 22402366).toLocaleString()}`);

    // Log each transaction for detailed analysis
    console.log('\n📋 Class A Transaction Details:');
    classATransactions.forEach((t, i) => {
      console.log(`  ${i + 1}. ${t.transaction_type} | ${t.credit_debit} | qty: ${t.share_quantity} | date: ${t.transaction_date}`);
    });
    console.log('🔍 === END CLASS A DEBUG ===\n');

    const totalCredits = transactions.filter(t => t.credit_debit === 'Credit').reduce((sum, t) => sum + Math.abs(Number(t.share_quantity || 0)), 0);
    const totalDebits = transactions.filter(t => t.credit_debit === 'Debit').reduce((sum, t) => sum + Math.abs(Number(t.share_quantity || 0)), 0);
                      ...
                      <TableBody>
                        {(() => {
                          ...
                          data.transactions.forEach((transaction) => {
                            ...
                            const quantity = Math.abs(Number(transaction.share_quantity) || 0);
                            let isDebit = false;
