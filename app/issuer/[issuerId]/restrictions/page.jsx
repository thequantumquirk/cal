...
export default function RestrictionsPage({ params: paramsPromise }) {
  ...
  // ⚡ SWR FETCHER with cache-busting headers
  ...
  // Balanced caching config - cache but allow revalidation
  const swrConfig = {
    revalidateOnFocus: false,      // Don't refetch on window focus
    revalidateOnReconnect: false,  // Don't refetch on reconnect
    revalidateOnMount: true,       // Fetch on mount if no cache
    dedupingInterval: 2000,        // Dedupe requests within 2 seconds (reduced from 5min)
    focusThrottleInterval: 60000,  // Throttle focus revalidation to 1min (reduced from 5min)
    refreshInterval: 0,            // No automatic refresh
    shouldRetryOnError: false,     // Don't retry on error
    errorRetryCount: 0,            // No retries
    revalidateIfStale: true,       // DO revalidate stale data (was false - this was the problem!)
  };
  ...
  // ⚡ FETCH TRANSACTION-BASED RESTRICTIONS: Get restrictions from transfers_new table
  const { data: transactionBasedRestrictions = [], mutate: mutateTransactionRestrictions } = useSWR(
  ...
  const pageLoading = issuerId && (
    ...
    transactionBasedRestrictions === undefined ||
    shareholders === undefined ||
    securities === undefined ||
    users === undefined
  );

  // ⚡ COMBINE RESTRICTIONS: Merge manual and transaction-based restrictions
  const combinedRestrictions = React.useMemo(() => {
    // Manual restrictions from transaction_restrictions_new
    const manualRestrictions = (shareholderRestrictions || []).map(r => ({
      ...r,
      source: 'manual',
      source_label: 'Manual'
    }));

    // Transaction-based restrictions from transfers_new (where restriction_id exists)
    const transactionRestrictions = (transactionBasedRestrictions || [])
      .filter(t => t.restriction_id) // Only transactions with restrictions
      .reduce((acc, transaction) => {
        // Group by shareholder + cusip + restriction_id to avoid duplicates
        const key = `${transaction.shareholder_id}-${transaction.cusip}-${transaction.restriction_id}`;

        if (!acc[key]) {
          acc[key] = {
            id: `tx-${key}`, // Unique ID for React key
            shareholder_id: transaction.shareholder_id,
            restriction_id: transaction.restriction_id,
            cusip: transaction.cusip,
            restricted_shares: transaction.share_quantity,
            restriction_date: transaction.transaction_date,
            source: 'transaction',
            source_label: 'Transaction',
            transaction_id: transaction.id,
            created_at: transaction.created_at,
            notes: `From transaction on ${new Date(transaction.transaction_date).toLocaleDateString()}`
          };
        } else {
          // If duplicate key, accumulate shares (in case of multiple transactions)
          acc[key].restricted_shares += transaction.share_quantity;
        }
    ...
    return [
      ...manualRestrictions,
      ...Object.values(transactionRestrictions)
    ];
  }, [shareholderRestrictions, transactionBasedRestrictions]);
  ...
  const updateRestrictionTemplate = async (templateId, templateData) => {
    try {
      ...
      // Update cache optimistically and send request
      const response = await fetch("/api/restriction-templates", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
      ...
      await mutateTemplates(
        ...
        { revalidate: false } // Don't revalidate since we're providing fresh data
      );
      ...
      // Revert optimistic update on error by forcing revalidation
  ...
  const createShareRestriction = async (restrictionData) => {
    try {
      ...
      if (response.ok) {
        // ⚡ Force immediate cache revalidation by fetching fresh data
        await mutateShareRestrictions(undefined, { revalidate: true });
  ...
  const createShareholderRestriction = async (restrictionData) => {
    try {
      ...
      if (response.ok) {
        ...
        // ⚡ Force immediate cache revalidation by fetching fresh data
        await mutateShareholderRestrictions(undefined, { revalidate: true });
