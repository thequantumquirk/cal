        ...
        workbook.SheetNames.forEach((sheetName) => {
          ...
          if (/recordkeeping book/i.test(sName)) {
            ...
            rows.slice(1).forEach((row) => {
              ...
              // Update context (including date for next row)
              lastContext = {
                issue_name: issueName,
                issue_ticker: ticker,
                trading_platform: platform,
                ...
                security_type: securityType,
                issuance_type: issuanceType,
                transaction_date: transaction_date || lastContext.transaction_date  // Preserve date for next row
              };
        ...
        <div className="bg-background border border-border rounded-2xl shadow-2xl p-8 max-w-lg w-full">
          ...
          <div className="space-y-3">
            ...
            <div className="relative w-full bg-muted rounded-full h-3 overflow-hidden border border-border/50">
              <div
                className="bg-wealth-gradient h-3 rounded-full transition-all duration-500 ease-out relative overflow-hidden"
          ...
          <div className="mt-6 p-3 bg-primary/5 border border-primary/20 rounded-lg">
            <p className="text-xs text-foreground/70 text-center">
              💡 <span className="font-semibold text-foreground">Tip:</span> This process may take a few moments depending on the file size
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-4">
      ...
      {formData && (
        <>
          ...
          {/* Missing Dates Warning - Critical for position calculations */}
          {validationWarnings && validationWarnings.missingDates.length > 0 && (
            <div className="bg-orange-500/10 border border-orange-500/30 rounded-lg p-4 flex items-start space-x-3 mb-4">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-orange-900 dark:text-orange-300">
                  {validationWarnings.missingDates.length} transaction(s) missing date - CRITICAL
                </p>
                <p className="text-xs text-orange-700 dark:text-orange-400 mt-1">
                  Transactions without dates will cause <strong>incorrect shareholder position calculations</strong> (e.g., split screen errors).
                  Go to the <strong>Transactions tab</strong> to edit and add the missing dates before importing.
                </p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {validationWarnings.missingDates.slice(0, 5).map((item, idx) => (
                    <span key={idx} className="inline-flex items-center px-2 py-1 bg-orange-500/20 text-orange-800 dark:text-orange-300 text-xs rounded">
                      Row {item.row}: {item.type} ({item.account})
                    </span>
                  ))}
                  {validationWarnings.missingDates.length > 5 && (
                    <span className="inline-flex items-center px-2 py-1 bg-orange-500/20 text-orange-800 dark:text-orange-300 text-xs rounded">
                      +{validationWarnings.missingDates.length - 5} more
                    </span>
                  )}
                </div>
                <button
                  onClick={() => setActiveTab("transactions")}
                  className="mt-2 text-xs font-medium text-orange-700 dark:text-orange-300 hover:text-orange-900 dark:hover:text-orange-100 underline"
                >
                  Go to Transactions tab to fix →
                </button>
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            {validationWarnings && (
              ...
              validationWarnings.missingCUSIPs?.length > 0
                ...
                <div className="flex items-center justify-end space-x-2 text-sm text-destructive">
                  <span className="font-medium">⚠️ Please fix validation errors before saving</span>
                </div>
              )}
            <div className="flex justify-end space-x-2">
              ...
              <Button
                ...
                disabled={loading || (validationWarnings && (
                  validationWarnings.emptyTransactionTypes?.length > 0 ||
                  validationWarnings.missingDates?.length > 0 ||
                  validationWarnings.zeroQuantities?.length > 0 ||
                  validationWarnings.missingShareholders?.length > 0 ||
