  ...
  await postMany(
    ...
    (s) => ({
      ...
      if (formData.restrictions && formData.restrictions.length > 0) {
        console.log(`📋 Saving ${formData.restrictions.length} restriction templates...`);
        await postMany(formData.restrictions, "/api/restriction-templates", "restrictions", (r) => ({
    ...
    if (formData.transactions && formData.transactions.length > 0) {
      ...
      if (!txRes.ok) {
        ...
        if (formData.transactions && formData.transactions.length > 0 && formData.securities && formData.securities.length > 0) {
          // Fetch securities from database to get their IDs
          const secRes = await fetch(`/api/securities?issuerId=${issuerId}`);
          const securities = await secRes.json().catch(() => []);

          // Build CUSIP → security_id mapping
          const cusipToSecurityMap = {};
          if (Array.isArray(securities)) {
            securities.forEach(sec => {
              if (sec.cusip && sec.id) {
                cusipToSecurityMap[sec.cusip] = sec.id;
              }
            });
          }

          console.log("📊 CUSIP to Security mapping:", cusipToSecurityMap);

          // Calculate position for each shareholder+security combination
          const positions = {}; // key: "shareholderId|securityId", value: balance
          ...
          formData.transactions.forEach(tx => {
            ...
            if (!positions[key]) {
              positions[key] = { shareholderId, securityId, balance: 0 };
            }
            ...
            positions[key].balance += (tx.share_quantity || 0) * multiplier;
          });
          ...
          const positionsPayload = Object.values(positions)
            ...
            .map(pos => ({
              ...
              position_date: new Date().toISOString().split('T')[0], // Today's date
            }));
              ...
              {tabs.map((tab) => (
                <button
                  ...
                  className={`
                    ...
                    ${tab.warnings > 0 ? "ring-2 ring-red-400 ring-offset-2" : ""}
                  `}
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className={`text-xs mt-1 ${activeTab === tab.key ? "text-orange-100" : "text-gray-500"}`}>
                        {tab.description}
                      </p>
                    </div>
                    <div className="ml-2">
                  ...
                  {tab.warnings > 0 && (
                    <div className="absolute -top-1 -right-1 flex items-center justify-center w-6 h-6 bg-red-500 text-white text-xs font-bold rounded-full border-2 border-white shadow-lg">
                      {tab.warnings}
                    </div>
                  )}
                </button>
    ...
    <div className="bg-white rounded-lg shadow-lg w-96 p-6">
      ...
      {conflictData && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg shadow-2xl w-[500px] max-w-[90vw]">
            <div className="flex items-start mb-4">
              ...
              <div className="flex-1">
                <h3 className="font-bold text-xl text-gray-900 mb-2">Issuer Already Exists</h3>
                <p className="text-sm text-gray-700 mb-4">
                  An issuer named <span className="font-semibold text-gray-900">{conflictData.issuer_name || conflictData.display_name}</span> already exists in the database.
                </p>
              </div>
            </div>

            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
              <p className="font-semibold text-red-800 mb-2">⚠️ Warning: This will DELETE all existing data</p>
