...
import { User, Bell, Search, Settings, Activity, Users, Building, ArrowRightLeft, Calendar, Shield, Key, ChevronDown, Plus, Home, Check } from "lucide-react"
  ...
  return (
    <header className="bg-card/80 backdrop-blur-xl shadow-lg border-b border-border/20">
      <div className="flex items-center justify-between px-4 sm:px-6 py-2">
          ...
          {(pathname === "/shareholder-home" || (userRole === "broker" && pathname.startsWith("/information"))) && (
            <div className="flex items-center -my-1">
              <img
                src="/logo.png"
                alt="Company Logo"
                className="w-36 h-auto sm:w-48 object-contain"
              />
            </div>
          )}
        ...
        {/* Issuer Switcher - only show for issuer-scoped pages or if superadmin has issuers */}
        {((isIssuerScoped && currentIssuer) || (userRole === 'superadmin' && availableIssuers.length > 0)) && (
          <div className="flex items-center">
            {(hasMultipleIssuers || (userRole === 'superadmin' && availableIssuers.length > 0)) ? (
              <DropdownMenu>
