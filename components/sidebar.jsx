...
import { useState, memo, useCallback, useTransition, useEffect } from "react";
...
function Sidebar({
  ...
  // Clear optimistic path once navigation completes
  useEffect(() => {
    // When pathname changes, clear the optimistic path
      ...
      {issuerScopedNavigation.length > 0 && (
        <div className="space-y-3">
          {issuerScopedNavigation.map((item) => {
            // CRITICAL: Only show ONE active item at a time
            // If we're navigating (optimisticPath exists), ONLY highlight the optimistic path
            // Otherwise, use the actual pathname
            const isActive = optimisticPath
              ? optimisticPath === item.href  // During navigation: only show new selection
              : pathname === item.href;       // After navigation: show actual path
            const isNavigating = isPending && optimisticPath === item.href;
