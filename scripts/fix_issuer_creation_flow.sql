-- ========== FIX ISSUER CREATION FLOW ==========
-- This script ensures proper issuer admin assignments and status updates

-- Step 1: Check current state of all issuers
SELECT '=== CURRENT ISSUERS STATE ===' as info;
SELECT 
  i.id,
  i.name,
  i.display_name,
...
-- Step 2: Check all users with issuer_admin role
SELECT '=== USERS WITH ISSUER_ADMIN ROLE ===' as info;
SELECT 
  u.id,
  u.email,
  u.role,
  u.created_at,
  CASE WHEN iu.id IS NOT NULL THEN 'HAS ISSUER' ELSE 'NO ISSUER' END as issuer_status,
  i.display_name as assigned_issuer
FROM users u
...
-- Step 3: Fix issuer admin assignments for issuers without admins
-- For each issuer without an admin, assign the first available issuer_admin user
...
-- Step 6: Show final issuer_users relationships
SELECT '=== FINAL ISSUER_USERS RELATIONSHIPS ===' as info;
SELECT 
  u.email,
  r.name as role_name,
