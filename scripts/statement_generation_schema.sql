...
CREATE POLICY statements_read_all ON shareholder_statements FOR SELECT TO authenticated USING (true);
CREATE POLICY statements_admin_write ON shareholder_statements FOR ALL TO authenticated 
  USING (public.current_user_role() = 'admin') 
  WITH CHECK (public.current_user_role() = 'admin');

-- Statement Details Policies
CREATE POLICY statement_details_read_all ON statement_details FOR SELECT TO authenticated USING (true);
CREATE POLICY statement_details_admin_write ON statement_details FOR ALL TO authenticated 
  USING (public.current_user_role() = 'admin') 
  WITH CHECK (public.current_user_role() = 'admin');

-- Statement Transactions Policies
CREATE POLICY statement_transactions_read_all ON statement_transactions FOR SELECT TO authenticated USING (true);
CREATE POLICY statement_transactions_admin_write ON statement_transactions FOR ALL TO authenticated 
  USING (public.current_user_role() = 'admin') 
  WITH CHECK (public.current_user_role() = 'admin');
...
BEGIN
  ...
  FOR v_cusip IN 
    ...
    WHERE issuer_id = p_issuer_id 
      AND shareholder_id = p_shareholder_id
      AND credit_date <= p_statement_date
      AND share_quantity > 0
  LOOP
    -- Get shares outstanding (calculate from transfer_journal)
    SELECT COALESCE(SUM(
      CASE 
        WHEN tj.credit_debit = 'Credit' THEN tj.share_quantity
        WHEN tj.credit_debit = 'Debit' THEN -tj.share_quantity
      END
    ), 0) INTO v_shares_outstanding
    FROM transfer_journal tj
    WHERE tj.issuer_id = p_issuer_id 
      AND tj.shareholder_id = p_shareholder_id
      AND tj.cusip = v_cusip 
      AND tj.credit_date <= p_statement_date
      AND tj.status = 'Active';
...
-- Trigger to update market_value_total when price_per_share changes
CREATE OR REPLACE FUNCTION update_market_value_total()
RETURNS TRIGGER AS $$
BEGIN
  -- This would typically be calculated based on total shares outstanding
  -- For now, we'll leave it as NULL and calculate it in the application
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trigger_update_market_value_total
  BEFORE INSERT OR UPDATE ON market_values
  FOR EACH ROW
  EXECUTE FUNCTION update_market_value_total();
...
CREATE TRIGGER update_shareholder_statements_updated_at
